...
import { releaseReservations } from "@/lib/inventory.server";
import { sendShippingNoticeEmail } from "@/lib/email.server";
...
export async function fulfilOrderItems(params: {
...
}): Promise<{ fulfillmentId: string; orderStatus: string }> {
  ...
  return { fulfillmentId: result.fulfillmentId, orderStatus: result.orderStatus };
}

...
async function CheckoutSummary({ userId, discountCode }: { userId: string; discountCode?: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-3xl border border-gray-200/80 dark:border-ink-800 bg-gray-50/70 dark:bg-ink-900 p-6 shadow-xs">
      <h3 className="font-sans text-sm font-bold uppercase tracking-wider text-gray-900 dark:text-white">Order Summary</h3>
      <div className="flex flex-col divide-y divide-gray-200/70 dark:divide-ink-800">
        {view.lines.map((line) => (
          <div key={line.itemId} className="flex justify-between gap-4 py-3 text-xs">
            <span className="font-medium text-gray-800 dark:text-gray-200">
              {line.productTitle} × {line.quantity}
            </span>
            <span className="font-mono font-semibold text-gray-900 dark:text-white">{formatMoney({ amount: line.lineTotal, currency })}</span>
          </div>
        ))}
      </div>
      <div className="flex flex-col gap-2 border-t border-gray-200 dark:border-ink-800 pt-3 text-xs text-gray-600 dark:text-gray-400">
        <div className="flex justify-between">
          <span>Subtotal</span>
          <span className="font-mono font-medium text-gray-900 dark:text-white">{formatMoney({ amount: view.subtotal, currency })}</span>
        </div>
        {discountAmount > 0 && (
        ...
        <div className="flex justify-between">
          <span>Tax</span>
          <span className="font-mono font-medium text-gray-900 dark:text-white">{formatMoney({ amount: taxAmount, currency })}</span>
        </div>
        <div className="flex justify-between border-t border-gray-200 dark:border-ink-800 pt-2.5 text-xs font-bold text-gray-900 dark:text-white">
          <span>Total</span>

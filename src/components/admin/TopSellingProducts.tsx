...
export function TopSellingProducts({ products: dynamicProducts }: TopSellingProductsProps) {
  ...
  const products = dynamicProducts && dynamicProducts.length > 0 ? dynamicProducts : defaultProducts;
    ...
    <div className="flex flex-col justify-between rounded-2xl border border-[#1E293B] bg-[#111827] p-6 shadow-panel">
      <div>
        <div className="flex items-center justify-between border-b border-[#1E293B] pb-3.5">
          ...
          <span className="font-mono text-[10px] text-indigo-400 font-semibold">By Net Sales</span>
        </div>
          ...
          {products.map((p, idx) => (
            <div key={idx} className="flex items-center justify-between py-3">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#1E293B] font-sans text-xs font-bold text-slate-300">
                  {idx + 1}
                </div>
                <div>
                  <h4 className="font-sans text-xs font-bold text-white line-clamp-1">{p.name}</h4>
                  <p className="font-sans text-[11px] text-slate-400">{p.category} · {p.orders} units sold</p>
                </div>
              </div>
              <span className="font-mono text-xs font-bold text-indigo-300 ml-2 whitespace-nowrap">{p.revenue}</span>
            </div>
          ))}
        </div>
      </div>
